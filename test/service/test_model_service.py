...
@pytest.fixture
def model_schema():
    return schemas.Model(name="Testmodel")
...
def test_create_model_success(mock_db, model_schema, mock_crud):
    ...
    mock_crud.get.assert_called_once_with(db=mock_db, id_=model_schema.name)
...
def test_get_model_success(mock_db, model_schema, mock_crud):
    ...
    result = model_service.get_model(mock_db, model_schema.name)
...
def test_get_model_not_found(mock_db, model_schema, mock_crud):
    ...
    with pytest.raises(HTTPException) as exc_info:
        model_service.get_model(mock_db, model_schema.name)
...
def test_get_models(mock_db, mock_crud):
    mock_crud.get_multi.return_value = [schemas.Model(name="model1"), schemas.Model(name="model2")]
...
def test_remove_model_success(mock_db, model_schema, mock_crud):
    ...
    result = model_service.remove_model(mock_db, model_schema.name)

    mock_crud.get.assert_called_once_with(mock_db, id_=model_schema.name)
    mock_crud.remove.assert_called_once_with(db=mock_db, id_=model_schema.name)
    assert result == model_schema
...
def test_remove_model_not_found(mock_db, model_schema, mock_crud):
    ...
    with pytest.raises(HTTPException) as exc_info:
        model_service.remove_model(mock_db, model_schema.name)
